...
template <typename T>
using radial_traits_ptr = std::unique_ptr<RadialTraits<T>>;
...
struct UnprunedSphericalGridSpecification {
  ...
  radial_traits_ptr<T> radial_traits; ///< Radial traits (order, scaling factors, etc)

  AngularQuad angular_quad; /// Angular quadrature specification
  AngularSize angular_size; /// Number of angular quadrature points

  UnprunedSphericalGridSpecification( RadialQuad rq, const RadialTraits<T>& traits,
...
struct PrunedSphericalGridSpecification {
  ...
  PrunedSphericalGridSpecification(RadialQuad rq, radial_traits_ptr<T>&& traits, Arg&&... arg) :
    radial_quad(rq), radial_traits(std::move(traits)), pruning_regions(std::forward<Arg>(arg)...) { }
  template <typename... Arg>
  PrunedSphericalGridSpecification(RadialQuad rq, const RadialTraits<T>& traits, Arg&&... arg) :
...
PrunedSphericalGridSpecification<T> robust_psi4_pruning_scheme(
  UnprunedSphericalGridSpecification<T>
);
...
PrunedSphericalGridSpecification<T> create_pruned_spec(
  PruningScheme, UnprunedSphericalGridSpecification<T>
);
...
struct SphericalGridFactory {
  ...
  static spherical_grid_ptr generate_unpruned_grid( RadialQuad, const RadialTraits<T>&,
    AngularQuad, AngularSize );
  static spherical_grid_ptr generate_pruned_grid( RadialQuad, const RadialTraits<T>&,
    const std::vector<PruningRegion>&);


  static inline spherical_grid_ptr generate_grid(
    UnprunedSphericalGridSpecification<T> gs) {
  ...
  static inline spherical_grid_ptr generate_grid(
    PrunedSphericalGridSpecification<T> gs) {
    if(!gs.radial_traits) throw std::runtime_error("RadialTraits Not Set");
...
// Out-of-line (but fully templated, so header-only-safe for any T) bodies
// for the runtime-dispatch overloads declared above. Included as a trailer
// rather than merged into this file to keep the RadialQuad/AngularQuad
// switch-based dispatch logic out of the class definition; each of these is
// itself entirely templates, so multiple TUs including this header for the
// same T is not an ODR problem (same reason radial_factory.hpp's in-class
// RadialFactory<T>::generate is safe).
